      ...
      <div className="mt-4 flex flex-col">
        {!!fields.length && (
          <DragDropContext onDragEnd={handleDrag}>
            <Droppable droppableId={`dropplable-${data.formKey}`}>
              {(provided:any) => (
                <div
                ...
                >
                  {fields.map((field, index) => {
                    ...
                    return (
                      <Draggable
                        key={`dragglable-item-${data.formKey}-${index}`}
                        draggableId={`dragglable-item-${data.formKey}-${index}`}
                        index={index}
                      >
                        {(provided) => (
                          <div
                          ...
                          >
                            ...
                            <div className="flex-1 flex flex-col justify-center px-3 cursor-pointer hover:bg-muted/80 transition-all">
                               ...
                               <p className="text-xs text-muted-foreground">{field[descriptionKey]}</p>
                            </div>
                          </div>
                        )}
                      </Draggable>
